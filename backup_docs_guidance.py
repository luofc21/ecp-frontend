#!/usr/bin/env python3
"""
前端资源中心（docs/guidance）文档备份工具
备份 http://frontend.pcitech.online/docs/guidance/ 下的所有文档页面

依赖安装:
    pip install requests beautifulsoup4 markdownify

用法:
    python backup_docs_guidance.py
"""
...
BASE_URL    = "http://frontend.pcitech.online/docs/guidance/"
OUTPUT_DIR  = "./ecp-ui-plus-docs/docs/guidance"
ENTRY_URL   = "http://frontend.pcitech.online/docs/guidance/induction/chapter-technic/architecture-technical/basics/micro-frontend.html"
REQUEST_DELAY   = 0.5
REQUEST_TIMEOUT = 15
...
def is_internal_doc(url: str) -> bool:
    # 属于 docs/guidance 域名下的文档页面（含 .html 或无扩展名的路径）
    if not url.startswith(BASE_URL):
        return False
    # 排除图片/PDF/CSS/JS 等静态资源
...
def url_to_output_path(url: str) -> str:
    path = urlparse(url).path
    path = re.sub(r"^/docs/guidance/", "", path)
    path = re.sub(r"\.html$", "", path)   # 去掉 .html 后缀
    path += ".md"
...
def extract_sidebar_links(soup: BeautifulSoup, base_url: str) -> list[str]:
    ...
    （此站点不使用 nav#VPSidebarNav，而是 div.nav-side-content）
    """
    ...
    # 找所有嵌套的 nav-side-item 和 nav-side-item-group
...
def main():
    ...
    print("=" * 60)
    print("  前端资源中心（docs/guidance）文档备份工具")
